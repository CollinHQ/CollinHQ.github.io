  ...
  return (
    <section className="px-6 py-20">
      <div className="max-w-4xl mx-auto">
        <p className="text-yellow-500 text-xs uppercase tracking-widest mb-2 text-center">Word of Mouth</p>
        <h2 className="font-serif text-4xl font-bold text-white mb-12 text-center">What People Say</h2>
        ...
        <div className={`grid gap-6 ${testimonials.length > 1 ? 'md:grid-cols-2' : 'max-w-2xl mx-auto'}`}>
          {testimonials.map(({ id, quote, name, title, company, relationship }) => (
            <figure key={id} className="bg-[#1a2535] rounded-2xl p-8 flex flex-col">
              <svg width="28" height="28" viewBox="0 0 24 24" fill="currentColor" className="text-yellow-500/40 mb-4">
